    ...
    <section className="mx-5 rounded-xl border border-mint-bd bg-mint-wash px-3.5 py-2.5">
      <div className="flex items-center justify-between">
        ...
        {!done && (
          <button
            ...
            className="shrink-0 rounded-full bg-ink px-3.5 py-1.5 text-[14px] font-medium text-bg"
          >
            Review
          </button>
        )}
      ...
      <Sheet open={open} onClose={() => setOpen(false)} title="This week's learning">
        <div className="flex flex-col gap-3 pt-1">
          <p className="text-[15px] leading-snug text-ink">{proposal.summary}</p>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              disabled={pending}
              onClick={() => decide("skip")}
              className="rounded-full border border-line px-5 py-2.5 text-[14px] text-ink2"
            >
              Skip
            </button>
          </div>
